...
int qtaguid_tagSocket(int sockfd, int tag, uid_t uid) {
    ...
    ALOGV("Tagging socket %d with tag %"PRIu64"{%u,0} for uid %d", sockfd, kTag, tag, uid);

    res = write_ctrl(lineBuf);
    if (res < 0) {
        ALOGI("Tagging socket %d with tag %"PRIu64"(%d) for uid %d failed errno=%d",
...
int qtaguid_deleteTagData(int tag, uid_t uid) {
    ...
    ALOGV("Deleting tag data with tag %"PRIu64"{%d,0} for uid %d", kTag, tag, uid);
    ...
    res = write_ctrl(lineBuf);
    if (res < 0) {
        ALOGI("Deleteing tag data with tag %"PRIu64"/%d for uid %d failed with cnt=%d errno=%d",
