					     ...
					     struct external_auth *params)
{
	...
	data = os_malloc(sizeof(struct external_auth) + extlen);
	snprintf((char *)data, sizeof(struct external_auth), "EXT_AUTH_STATUS ");
	os_memcpy(data + extlen, params, sizeof(struct external_auth));
	WPA_PUT_BE64((data + 42 + extlen), params->ssid_len);
	WPA_PUT_BE32((data + 58 + extlen), params->key_mgmt_suite);
	WPA_PUT_BE16((data + 62 + extlen), params->status);

	wpa_hexdump(MSG_DEBUG, "nl80211: data dump",
		data, sizeof(struct external_auth) + extlen);

	ret = wpa_driver_nl80211_driver_cmd(priv, (char *)data,
		(char *)data, sizeof(struct external_auth) + extlen);
	os_free(data);
#else
