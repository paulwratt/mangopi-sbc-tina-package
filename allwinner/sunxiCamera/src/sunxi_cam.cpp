...
bool SunxiCam::setExposureGain(int exp_val, int gain_val)
{
	if(state != CAM_STREAMON)
		return false;

	setexposuregain(camera, exp_val, gain_val);

	return true;
}

bool SunxiCam::getExposureGain(int *exposure, int *gain)
{
	if(state != CAM_STREAMON)
		return false;

	getexposuregain(camera, exposure, gain);

	return true;
}

bool SunxiCam::setAutoExposureGain(void)
{
	if(state != CAM_STREAMON)
		return false;

	setautoexposuregain(camera);
