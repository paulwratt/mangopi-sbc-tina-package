...
int key_data_read_by_name(const char *item_name, char *buffer, int buffer_len,
		  int *data_len)
{
	return sunxi_secure_object_read_by_name(item_name, buffer,
		buffer_len, data_len);
...
{
	return sunxi_secure_object_write_by_name(item_name, buffer, length);
}

int key_data_erase_by_name(const char *item_name)
{
	return sunxi_secure_object_erase_by_name(item_name);
}
