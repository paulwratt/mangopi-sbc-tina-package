...
#define SECKV_READ                        _IO('V', 25)
#define SECKV_WRITE                       _IO('V', 26)
#define SECKV_ERASE                       _IO('V', 27)
...
/*nand secure storage read/write*/
static int _nand_read_by_name(const char* name, unsigned char *buffer,
				     int buffer_len, int *data_len)
{
	int fd;
	int ret;
	char *dev;
	struct seckv_op_t *seckv_op;
	...
	seckv_op = (struct seckv_op_t *)malloc(sizeof(*seckv_op));
	if (!seckv_op) {
		ALOGE("Out of memory\n");
		close(fd);
		return -1;
	...
	seckv_op->key = name;
	seckv_op->key_buf_len = strlen(name);
	seckv_op->value = (unsigned char *)buffer;
	seckv_op->value_buffer_len = buffer_len;
	seckv_op->value_len = data_len;

	ret = ioctl(fd, SECKV_READ, seckv_op);
	if (ret < 0) {
		ALOGE("Nand seckv read fail\n");
		free(seckv_op);
		close(fd);
		return -1;
	}
...
static int _nand_write_by_name(const char *name, char *buffer,
			int length)
{
	int fd;
	int ret;
	...
	seckv_op->key_buf_len = strlen(name);
	seckv_op->value = (unsigned char *)buffer;
	seckv_op->value_buffer_len = length;
	seckv_op->value_len = NULL;

	ret = ioctl(fd, SECKV_WRITE, seckv_op);
	if (ret < 0) {
		ALOGE("Nand seckv write fail\n");
		free(seckv_op);
		close(fd);
		return -1;
	}
...
static int _nand_erase_by_name(const char *name)
{
	int fd;
	int ret;
	char *dev;
	...
	seckv_op->key_buf_len = strlen(name);
	seckv_op->value = NULL;
	seckv_op->value_buffer_len = 0;
	seckv_op->value_len = NULL;

	ret = ioctl(fd, SECKV_ERASE, seckv_op);
	if (ret < 0) {
		ALOGE("Nand seckv erase fail\n");
		free(seckv_op);
		close(fd);
		return -1;
	}
...
static int nv_read_by_name(const char* name, unsigned char *buffer,
				     int buffer_len, int *data_len)

{
	return _nand_read_by_name(name, buffer, buffer_len, data_len);
}
...
static int nv_write_by_name(const char *name, char *buffer,
			int length)
{
	return _nand_write_by_name(name, buffer, length);
}

static int nv_erase_by_name(const char *name)
{
	return _nand_erase_by_name(name);
}
...
static int sunxi_secstorage_read_by_name(const char* name, unsigned char *buffer,
				     int buffer_len, int *data_len)
{
	return nv_read_by_name(name, buffer, buffer_len, data_len);
}
...
static int sunxi_secstorage_write_by_name(const char *name, char *buffer,
				      int length)
{
	return nv_write_by_name(name, buffer, length);
}

static int sunxi_secstorage_erase_by_name(const char *name)
{
	return nv_erase_by_name(name);
}
...
static int sunxi_secure_storage_read_by_name(const char *item_name, char *buffer,
				     int buffer_len, int *data_len)
{
	return sunxi_secstorage_read_by_name(item_name, buffer, buffer_len, data_len);
}
...
static int sunxi_secure_storage_write_by_name(const char *item_name, char *buffer,
				      int length)
{
	return sunxi_secstorage_write_by_name(item_name, buffer, length);
}

int sunxi_secure_object_erase_by_name(const char *item_name)
{
	return sunxi_secstorage_erase_by_name(item_name);
}
...
int sunxi_secure_object_read_by_name(const char *item_name, char *buffer,
			     int buffer_len, int *data_len)
{
	return sunxi_secure_storage_read_by_name(item_name, buffer, buffer_len,
				      data_len);
}
...
int sunxi_secure_object_write_by_name(const char *item_name, char *buffer, int length)
{
	return sunxi_secure_storage_write_by_name(item_name, buffer, length);
}
