...
int main(int argc, char **argv)
{
	...
	if (!strncmp(argv[1], "-r", 2)) {
		...
		if (argv[2]) {	/* api read */
			ret = key_data_read_by_name(argv[2], buf, 256, &retLen);
			if (ret < 0) {
				printf("[api_by_name] Read %s fail\n", argv[2]);
			} else {
				printf("[api_by_name] %s = %s\n", argv[2], buf);
			}
		}
		...
		if (argv[2] && argv[3]) {
			ret =
			    key_data_write_by_name(argv[2], argv[3],
					   strnlen(argv[3], 128));
			if (ret < 0) {
				printf("[api_by_name]Write %s fail\n", argv[2]);
				goto error;
			}
		}
