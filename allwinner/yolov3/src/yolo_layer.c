...
int num_detections(layer layers_params[],float thresh)
{
    int i;
    int s=0;
    for(i=0;i<3;++i){
...
detection* make_network_boxes(layer layers_params[],float thresh,int* num)
{
...
void fill_network_boxes(layer layers_params[],int w,int h,int netw, int neth,float thresh, float hier, int *map,int relative,detection *dets)
{
    int j;
    for(j=0;j<3;++j){
...
detection* get_network_boxes(layer layers_params[],
...
detection* get_detections(blob blobs[],int img_w,int img_h, int netw, int neth, int *nboxes, int classes, float thresh, float nms)
{
    //int classes = 80;
    //float thresh = 0.5;
    ...
    layer layers_params[3];
    for(int i=0;i<3;++i){
        layer l_params = make_yolo_layer(1,blobs[i].w,blobs[i].w, 3, 9, classes);
        layers_params[i] = l_params;
