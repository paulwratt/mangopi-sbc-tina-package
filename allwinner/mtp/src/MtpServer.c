...
static MtpResponseCode doGetObject(struct MtpServer *mServer)
{
	...
	DLOG("fileLength=%"PRIu64"", fileLength);
...
static MtpResponseCode doSendObject(struct MtpServer *mServer)
{
	...
	if (access(mServer->mSendObjectFilePath, F_OK) == 0) {
		DLOG("%s exist", mServer->mSendObjectFilePath);
	...
	DLOG("initialData=%d, ret=%d, mSendObjectFileSize=%d(0x%x)\n",
	...
	if (ret < 0) {
		...
		if (mServer->mSendObjectFileSize - initialData > 0) {
			...
			if (ret < 0 && errno == ECANCELED)
				is_canceled = 1;
			DLOG("MTP_RECEIVE_FILE returned %d, errno=%d\n", ret, errno);
		}
	}
	/*
...
static MtpResponseCode doSendPartialObject(struct MtpServer *mServer)
{
	...
	DLOG("receiving partial %s offset:%"PRIu64" length:%u", edit->mPath, offset, length);
...
static MtpResponseCode doGetObjectPropValue(struct MtpServer *mServer)
{
	...
	DLOG("GetObectPropValue %u %s", handle, getObjectPropCodeName(property));
...
void MtpServerRelease(struct MtpServer *mServer)
{
	...
	if (mServer->mDevHandle != NULL)
		MtpDevHandleRelease(mServer->mDevHandle);
