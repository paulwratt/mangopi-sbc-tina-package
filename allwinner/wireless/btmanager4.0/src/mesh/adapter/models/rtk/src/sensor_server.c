...
static mesh_msg_send_cause_t sensor_server_send(const mesh_model_info_p pmodel_info,
                                                uint16_t dst, uint8_t app_key_index, void *pmsg, uint16_t msg_len)
{
    mesh_msg_t mesh_msg;
                                                      ...
                                                      uint16_t property_id)
{
    ...
    else
    {
        ...
        {
            timer_addr = (uint32_t)&pmsg->descriptors[0];
            *((sensor_descriptor_t *)(timer_addr)) = *pdescriptor;
        }
    }
...
bool sensor_server_reg(uint8_t element_index, mesh_model_info_p pmodel_info)
{
    ...
    pmodel_info->model_id = RTK_MESH_MODEL_SENSOR_SERVER;
