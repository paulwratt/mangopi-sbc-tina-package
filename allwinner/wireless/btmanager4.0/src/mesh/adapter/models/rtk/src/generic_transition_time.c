...
#define printi(FMT, ...) l_info(FMT, ##__VA_ARGS__)

#define plt_free(p, ram_type)                                       free(p)

#define MESH_MIN_TIMER_MS   100

typedef void (*tick_timeout_cb)(void);
typedef void(*timer_timeout_handler)(struct l_timeout *timeout,void * p_context);

struct l_timeout *g_timer_handler = NULL;
timer_timeout_handler timeout_handler;
...
static void mesh_tick_timer_stop()
{
    l_timeout_remove(g_timer_handler);
...
static bool mesh_tick_timer_is_running(void)
{
...
static void timer_timeout_handle(struct l_timeout *timeout,void * p_context)
{
    trans_time_timeout_handle();
    if(g_timer_handler != NULL)
    {
        l_timeout_modify_ms(timeout, MESH_MIN_TIMER_MS);
    }
}
...
void mesh_tick_timer_start(uint32_t tick_ms, tick_timeout_cb tick_cb)
{
    if(g_timer_handler == NULL)
    {
        g_timer_handler = l_timeout_create_ms(tick_ms,&timer_timeout_handle, NULL, NULL);
    }
}
                                    ...
                                    generic_transition_step_change_cb step_change)
{
    ...
    if (!mesh_tick_timer_is_running())
    {
        ...
        {
            mesh_tick_timer_start(MESH_MIN_TIMER_MS, trans_time_timeout_handle);
            ret = TRUE;
        }
    }
    else
