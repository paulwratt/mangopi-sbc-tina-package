...
/** @addtogroup Mesh_Node Mesh Node
  * @brief
  * @{
  */

/** @defgroup Mesh_Node_Exported_Types Mesh Node Exported Types
  * @brief
  * @{
  */
#if 1
#if (RFU_USED == 1)
typedef uint32_t plt_timer_t;
typedef uint32_t plt_queue_t;
typedef uint32_t prov_oob_info_t;
#define printw(FMT, ...) l_info(FMT, ##__VA_ARGS__)
#endif
typedef enum
{
    RTK_MESH_ROLE_DEVICE,
    RTK_MESH_ROLE_PROVISIONER
} mesh_role_t;
...
typedef struct
{
    ...
    uint8_t udb_interval; //!< unit: 100 millisecond, 0: use stack default value
    uint8_t snb_interval; //!< unit: 100 millisecond, 0: use stack default value
    uint8_t prov_interval; //!< unit: 100 millisecond, 0: use stack default value
    uint8_t proxy_interval; //!< unit: 100 millisecond, 0: use stack default value
    uint8_t identity_interval; //!< unit: 100 millisecond, 0: use stack default value
} mesh_node_cfg_t, *mesh_node_cfg_p;

typedef struct
{
    uint32_t used: 1; //!< flag used for flash
    uint32_t role: 1; //!< 1: provisioner, 0: node
    ...
    uint32_t flash_rpl: 1; //!< rpl has an independant flash storage control above flash bit
...
typedef int32_t (*model_data_cb_pf)(const mesh_model_info_p pmodel_info, uint32_t type,
                                    void *pargs);
typedef void (*model_notify_update_cb_pf)(void);
typedef bool (*model_opcode_cb_pf)(uint16_t opcode);
typedef struct _mesh_model_info_t
{
    ...
    model_opcode_cb_pf is_opcode_of_model;
...
typedef struct _mesh_model_t
{
    ...
    //uint8_t app_key_index; //key maybe deleted
    plt_timer_t pub_timer;
    uint8_t *app_key_binding; //!< dumm...
    plt_queue_t sub_queue; //!< subscribe addr list
...
typedef struct _mesh_element_t
{
    ...
    plt_queue_t model_queue; //!< @ref mesh_model_t
...
typedef struct
{
    ...
    uint8_t udb_interval; //!< 100ms
    uint8_t snb_interval; //!< 100ms
    uint8_t prov_interval; //!< 100ms
    uint8_t proxy_interval; //!< 100ms
    uint8_t identity_interval; //!< 100ms
    uint16_t unicast_addr;
    plt_queue_t element_queue; //!< @ref mesh_element_t
    uint8_t model_num;
    ...
    /** net parameters */
    uint8_t net_trans_count : 3; //!< send net_trans_count + 1 times
    uint8_t net_trans_steps : 5;
    uint8_t relay_retrans_count : 3; //!< send relay_retrans_count + 1 times
    uint8_t relay_retrans_steps : 5; //!< relay_retrans_steps * 10ms
    /** trans parameters */
...
bool app_key_update(uint16_t app_key_index, uint16_t app_key_index_g, const uint8_t app_key[16]);
...
#else
#define mesh_model_pub_period_get(pmodel) 0
#endif
