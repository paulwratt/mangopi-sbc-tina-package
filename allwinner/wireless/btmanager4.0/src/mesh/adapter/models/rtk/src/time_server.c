                                  ...
                                  uint8_t app_key_index, tai_time_t time)
{
    ...
    timer_addr = (uint32_t)&msg.tai_seconds[0];
    *((tai_time_t *)(timer_addr)) = time;
                                       ...
                                       uint8_t app_key_index, time_zone_t time_zone)
{
    ...
    timer_addr = (uint32_t)&msg.time_zone_offset_current;
    *((time_zone_t *)(timer_addr)) = time_zone;
...
mesh_msg_send_cause_t tai_utc_delta_status(mesh_model_info_p pmodel_info, uint16_t dst,
                                           uint8_t app_key_index, tai_utc_delta_t time_tai_utc_delta)
{
...
static bool time_server_receive(mesh_msg_p pmesh_msg)
{
    ...
    case MESH_MSG_TIME_GET:
        if (pmesh_msg->msg_len == sizeof(time_get_t))
        {
            ...
            time_status(pmodel_info, pmesh_msg->src, pmesh_msg->app_key_index, get_data);
        }
        break;
    case MESH_MSG_TIME_STATUS:
        if (pmesh_msg->msg_len == sizeof(time_status_t))
        {
            ...
            {
                /** publish time */
                tai_time_addr = (uint32_t)&pmsg->tai_seconds[0];
                time_publish(pmodel_info, *((tai_time_t *)(tai_time_addr)));
            }
        }
        break;
...
bool time_server_reg(uint8_t element_index, mesh_model_info_p pmodel_info)
{
    ...
    pmodel_info->model_id = RTK_MESH_MODEL_TIME_SERVER;
