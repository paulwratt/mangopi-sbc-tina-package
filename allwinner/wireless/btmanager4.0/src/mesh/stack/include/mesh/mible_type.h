...
typedef struct {
	uint16_t scan_interval; // Range: 0x0004 to 0x4000 Time = N * 0.625 msec
				// Time Range: 2.5 msec to 10.24 sec
	uint16_t scan_window;   // Range: 0x0004 to 0x4000 Time = N * 0.625 msec
				// Time Range: 2.5 msec to 10.24 seconds
	uint16_t timeout; // Scan timeout between 0x0001 and 0xFFFF in seconds,
...
typedef struct {
	uint16_t min_conn_interval; // Range: 0x0006 to 0x0C80, Time = N * 1.25
				    // msec, Time Range: 7.5 msec to 4 seconds.
	uint16_t max_conn_interval; // Range: 0x0006 to 0x0C80, Time = N * 1.25
				    // msec, Time Range: 7.5 msec to 4 seconds.
	uint16_t slave_latency;     // Range: 0x0000 to 0x01F3
	uint16_t conn_sup_timeout;  // Range: 0x000A to 0x0C80, Time = N * 10
	// msec, Time Range: 100 msec to 32 seconds
...
typedef enum {
	MIBLE_GAP_EVT_CONNECTED = MIBLE_GAP_EVT_BASE, /**< Generated when a
							 connection is
							 established.*/
	MIBLE_GAP_EVT_DISCONNECTED, /**< Generated when a connection is
				       terminated.*/
...
typedef struct {
	...
	mible_gatts_char_desc_cpf_t *char_format; // See more details at
						  // Bluetooth SPEC 4.2 [Vol 3,
						  // Part G] Page 539
...
typedef struct {
	...
	uint16_t
	    char_value_handle; // [out] where the assigned handle be stored.
...
typedef enum {
	MIBLE_GATTS_EVT_WRITE =
	    MIBLE_GATTS_EVT_BASE,	 // When this event is called, the
					  // characteristic has been modified.
	MIBLE_GATTS_EVT_READ_PERMIT_REQ,  // If charicteristic's rd_auth = TRUE,
					  // this event will be generated.
	MIBLE_GATTS_EVT_WRITE_PERMIT_REQ, // If charicteristic's wr_auth = TRUE,
					  // this event will be generated,
					  // meanwhile the char value hasn't
					  // been modified.
 ...
 * */
typedef struct {
	bool succ; // true: exist the specified characteristic and return
		   // correctly
...
typedef enum {
	DISABLE_NOTIFICATION_VALUE =
	    0x00, /*Value used to disable notifications or indicatinos*/
	ENABLE_NOTIFICATION_VALUE =
	    0x01, /*Value used to enable notification for a CCCD*/
	ENABLE_INDICATION_VALUE =
	    0x02, /*Value used to enable indication for a CCCD*/
