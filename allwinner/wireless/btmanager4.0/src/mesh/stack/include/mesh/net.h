...
#define MESH_NET_NO_ERROR           0
#define MESH_NET_FREE               1
#define MESH_NET_FRIEND_DISABLE     2
...
void mesh_net_set_iv_upd_test_mode(struct mesh_net *net,bool iv_upd);
...
uint32_t log_to_uint32(uint8_t log, uint8_t offset);
int hb_subscription_set(struct mesh_net *net, uint16_t src,
					uint16_t dst, uint8_t period_log);
void hb_sub_timeout_func(struct l_timeout *timeout, void *user_data);
void update_hb_pub_timer(struct mesh_net *net,
						struct mesh_net_heartbeat *hb);
bool cfg_srv_pkt(uint16_t src, uint32_t dst, uint16_t unicast,
