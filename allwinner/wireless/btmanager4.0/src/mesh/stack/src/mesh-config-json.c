...
//compatible json-1.2
static void jarray_string_del(json_object *jarray, char *str, size_t len,json_object *jmodel,const char *obj_name)
{
	int i, sz = json_object_array_length(jarray);
	...
	json_object_object_del(jmodel, obj_name);
	json_object_object_add(jmodel, obj_name, jarray_new);
}
#else
//need json-1.31
...
#ifdef CONFIG_XR829_BT
//compatible json-1.2
static void jarray_key_del(json_object *jarray, int16_t idx, json_object *jnode, const char *obj_name)
{
	json_object *jarray_new;
	int i, sz = json_object_array_length(jarray);
	...
	json_object_object_del(jnode, obj_name);
	json_object_object_add(jnode, obj_name, jarray_new);
...
bool mesh_config_net_key_del(struct mesh_config *cfg, uint16_t idx)
{
	...
	XR_COMPATIBLE_JSON_V1_2_KEY_DEL_PATCH(jarray, idx, jnode, "netKeys");
								...
								uint16_t idx)
{
	...
	XR_COMPATIBLE_JSON_V1_2_KEY_DEL_PATCH(jarray, idx, jnode, "appKeys");
							...
							uint16_t app_idx)
{
    ...
    XR_COMPATIBLE_JSON_V1_2_STR_DEL_PATCH(jarray, buf, 4, jmodel, "bind");
...
#ifdef CONFIG_XR829_BT
void update_element_config_jscon(struct mesh_config *cfg, struct mesh_config_node *node)
{
	//struct mesh_config_modes *modes = &node->modes;
	const struct l_queue_entry *entry;
	json_object *jnode, *jelems;//, *jelement;

	jnode = cfg->jnode;
	if (!json_object_object_get_ex(jnode, "elements", &jelems))
		return;

	if (jelems) {
		json_object_put(jelems);
		json_object_object_del(jnode, "elements");
		l_debug("update_element_config_json json_object_put jelems\n");
	}
	...
	l_debug("save node ele num %d to attach ", l_queue_length(node->elements));
					...
					struct mesh_config_node *node)
{
	...
	l_debug("create_config cfg->write_time.tv_sec %ld cfg->write_time.tv_usec %ld",cfg->write_time.tv_sec, cfg->write_time.tv_usec);
	return cfg;
}
						...
						struct mesh_config_sub *sub)
{
    ...
    XR_COMPATIBLE_JSON_V1_2_STR_DEL_PATCH(jarray, buf, 4, jmodel, "subscribe");
				...
				mesh_config_node_func_t cb, void *user_data)
{
	...
	if (fd < 0) {
		l_info("load_node fd<0");
		return false;
	}

	if (fstat(fd, &st) == -1) {
		close(fd);
		l_info("load_node fstat <0");
		return false;
	}
	...
	if (result) {
		...
		cfg->write_seq = node.seq_number;
		gettimeofday(&cfg->write_time, NULL);
		l_debug("load_node cfg->write_time.tv_sec %ld cfg->write_time.tv_usec %ld cfg->write_seq %d",cfg->write_time.tv_sec, cfg->write_time.tv_usec, cfg->write_seq);
...
static void idle_save_config(void *user_data)
{
	...
	l_debug("idle_save_config write_time.tv_sec %ld write_time.tv_usec %ld", info->cfg->write_time.tv_sec, info->cfg->write_time.tv_usec);
	if (info->cb)
								...
								void *user_data)
{
	...
	while ((entry = readdir(cfgdir)) != NULL) {
		...
		if (!load_node(fname, uuid, cb, user_data)) {
			...
			l_debug("mesh_config_load_nodes fail and load bak node");
