...
#else
struct mesh_node *scan_node;
static struct l_timeout *scan_timeout;
struct add_data *add_pending;
#endif
					...
					struct mesh_prov_node_info *info)
{
	...
	if (add_pending->primary >= (info->unicast + info->num_ele)) {
		add_pending->primary = info->unicast;
	} else {
		add_pending->primary = info->unicast;
		if(!keyring_put_prov_unicast(add_pending->node, info->unicast + info->num_ele)) {
			l_debug("keyring_put_prov_unicast eroor add_pendin->primary %d num_ele %d", add_pending->primary, add_pending->num_ele);
		}
	}
	...
	dbus_append_byte_array(builder, add_pending->uuid, 16);
#ifdef CONFIG_XR829_BT
	dbus_append_byte_array(builder, info->device_key, 16);
#endif
...
static bool add_data_get(void *user_data, uint8_t num_ele)
{
	...
	if (!keyring_get_prov_unicast(add_pending->node, &prov_unicast))
	{
		l_debug("keyring_get_prov_unicast fail prov_unicast %d", prov_unicast);
	}

	if (pending != add_pending)
		return false;

	add_pending->primary = prov_unicast;
	dbus = dbus_get_bus();
	...
	l_debug("prov_unicast %d num_ele %d", prov_unicast, num_ele);
	l_dbus_message_set_arguments(msg, "qy", prov_unicast, num_ele);
						...
						void *user_data)
{
	...
	if (!l_dbus_message_iter_get_fixed_array(&iter_key, &key, &n)
		...
		return dbus_error(msg, MESH_ERROR_INVALID_ARGS,
							"Bad device key");
	xr_info(TRC_PROV,"import_node_call: primary %x num_ele %d\n", primary, num_ele);
	xr_print_packet("import_node_call device_key: ", key, 16);
	if (!keyring_put_remote_dev_key(node, primary, num_ele, key))
		return dbus_error(msg, MESH_ERROR_FAILED, NULL);

	if (!keyring_get_prov_unicast(node, &prov_unicast)) {
		l_debug("get prov_unicast fail\n");
	}

	if ((primary + num_ele) > prov_unicast) {
		prov_unicast = primary + num_ele;
		if (!keyring_put_prov_unicast(node, prov_unicast)) {
			l_debug("put pro_unicast %d fail", prov_unicast);
		}
		l_debug("save prov_unicast %d", prov_unicast);
	}
	return l_dbus_message_new_method_return(msg);
}
...
#ifdef CONFIG_XR829_BT
	xr_info(TRC_PROV,"delete_node_call primary %d num_ele %d\n", primary, num_ele);
#endif
					...
					const uint8_t *data, uint16_t len)
{
    ...
    xr_print_packet("prov beacon rx:",data,len);
	...
	dbus_append_byte_array(builder, info->addr, MESH_BLE_ADDR_LEN);
	l_dbus_message_builder_finalize(builder);
	l_dbus_message_builder_destroy(builder);
						...
						void *user_data)
{
	struct mesh_node *node = user_data;
	const char *sender = l_dbus_message_get_sender(msg);
	xr_info(TRC_MGR,"cancel_scan_call\n");
						...
						void *user_data)
{
	...
	l_getrandom(key, sizeof(key));
	XR_CREATE_APP_CALL_PATCH(node,net_idx,app_idx,key);
						...
						void *user_data)
{
    ...
    if(add_pending->node != node)
        return dbus_error(msg, MESH_ERROR_BUSY, NULL);

    prov_disc_cb(dbus,user_data);

	return l_dbus_message_new_method_return(msg);
}

static struct l_dbus_message *local_key_mgr(struct l_dbus *dbus,
						struct l_dbus_message *msg,
						void *user_data)
{
	struct mesh_node *node = user_data;
    ...
    //struct mesh_net *net = node_get_net(node);
	//uint16_t net_idx;//, app_idx;
    lcl_key_mgr_t *p_key_mgr;
    //uint8_t *app_key;
    uint32_t n,msg_err = 0;

	if (!l_dbus_message_get_arguments(msg, "ay", &iter_key))
		return dbus_error(msg, MESH_ERROR_INVALID_ARGS, NULL);


	if (!l_dbus_message_iter_get_fixed_array(&iter_key, &p_key_mgr, &n)
								|| n != sizeof(lcl_key_mgr_t))
		return dbus_error(msg, MESH_ERROR_INVALID_ARGS,
							"Bad local key mgr");

    l_info("local key mgr: opcode %x,index(%d,%d),ele %d,mod %d,key %s",p_key_mgr->opcode,p_key_mgr->netkey_idx    \
        ,p_key_mgr->appkey_idx,p_key_mgr->element_addr,p_key_mgr->model_idx,getstr_hex2str(p_key_mgr->key,16));
    msg_err = lcl_key_ctl(node,p_key_mgr);

    if(msg_err)
    {
        dbus_error(msg, msg_err, NULL);
    }
	return l_dbus_message_new_method_return(msg);
}
#endif
static void setup_management_interface(struct l_dbus_interface *iface)
{
	...
	l_dbus_interface_method(iface, "SetKeyPhase", 0, set_key_phase_call, "",
						"qy", "net_index", "phase");
#ifdef CONFIG_XR829_BT
	l_dbus_interface_method(iface, "MgrLclKey", 0, local_key_mgr,
					"", "ay", "", "net_index", "app_index");
