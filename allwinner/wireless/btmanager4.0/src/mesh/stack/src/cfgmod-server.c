					...
					bool vendor, bool unreliable)
{
	...
	l_debug("pub_set: status %d, ele_addr %4.4x, ota: %4.4x, mod: %x, idx: %3.3x pub_addr %x",
					status, ele_addr, ota, mod_id, idx, l_get_le16(pub_addr));
					...
					bool virt, uint32_t opcode)
{
	...
	case OP_CONFIG_MODEL_SUB_ADD:
        l_debug("%s ele_addr %x mod_id %x addr %p virt %x grp %x",__func__,ele_addr,mod_id,addr,virt,grp);
...
static void hb_pub_timeout_func(struct l_timeout *timeout, void *user_data)
{
	...
	l_debug("%s %d left",__func__,hb->pub_count);
}
...
#endif
{
	...
	case OP_DEV_COMP_GET:
		...
		n += node_generate_comp(node, long_msg + n,
							CFG_MAX_MSG_LEN - n);
        //l_debug("%s composition get %s",__func__,getstr_hex2str(long_msg,n));
		break;
	...
	case OP_CONFIG_MODEL_PUB_GET:
        ...
        l_debug("%s pub get %s",__func__,getstr_hex2str(long_msg,n));
		break;
	...
	case OP_APPKEY_GET:
		...
		n += (size + 3);
        l_debug("%s appkey get %s",__func__,getstr_hex2str(long_msg,n));
		break;
