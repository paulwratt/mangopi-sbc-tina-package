...
#include "src/bluez-5.54_shared/ecc.h"
#else
...
enum int_state {
	...
	INT_PROV_DATA_ACKED,
#ifdef CONFIG_XR829_BT
	INT_PROV_REQUEST_CHOOSE_CAP,
	INT_PROV_RECV_CHOOSE_AUTH,
	INT_PROV_REQUEST_PUBLIC_KEY,
	INT_PROV_RECV_PUBLIC_KEY,
#endif
};
...
#ifdef CONFIG_XR829_BT
	uint8_t prov_manaual;
#endif
};

static struct mesh_prov_initiator *prov = NULL;
static uint8_t prov_manaual_global = 0;
				...
				void *trans_data, uint8_t transport)
{
	...
	bluez554_ecc_make_key(prov->conf_inputs.prv_pub_key, prov->private_key);
							...
							uint8_t *secret)
{
	...
	if (!bluez554_ecdh_shared_secret(tmp, priv, secret))
		return false;
...
static void pub_key_cb(void *user_data, int err, uint8_t *key, uint32_t len)
{
...
#ifdef CONFIG_XR829_BT
	uint8_t *out;
	prov->state = INT_PROV_RECV_PUBLIC_KEY;
	out = l_malloc(1 + sizeof(prov->conf_inputs.start));
	out[0] = PROV_START;
	memcpy(out + 1, &prov->conf_inputs.start,
				sizeof(prov->conf_inputs.start));

	l_info("PROV_CAPS send prov_start pub_key %d auth %d action %d size %d", prov->conf_inputs.start.pub_key, prov->conf_inputs.start.auth_method, prov->conf_inputs.start.auth_action, prov->conf_inputs.start.auth_size);
	...
	if (err || !key || len != 64) {
		l_debug("pub_key_cb error err %d len %d", err, len);
		msg.opcode = PROV_FAILED;
		msg.reason = PROV_ERR_UNEXPECTED_ERR;
		prov->trans_tx(prov->trans_data, &msg, sizeof(msg));
		return;
...
static void int_prov_auth(void)
{
	...
	l_debug("auth_method: %d  action %d action size %d", prov->conf_inputs.start.auth_method, prov->conf_inputs.start.auth_action, prov->conf_inputs.start.auth_size);
				...
				struct prov_start *start)
{
	...
	l_debug("auth_method %d auth_action %d auth_size %d", start->auth_method, start->auth_action, start->auth_size);
}

#ifdef CONFIG_XR829_BT
static void choose_caps_cb(void *user_data, int err, uint8_t *key, uint32_t len)
{
	struct prov_fail_msg msg;
	//uint8_t fail_code[2];
	uint8_t *out;
	...
	if (err || !key || len != 5) {
		...
		msg.reason = PROV_ERR_UNEXPECTED_ERR;
		prov->trans_tx(prov->trans_data, &msg, sizeof(msg));
		l_debug("err %d len %d", err, len);
		return;
	}

	prov->conf_inputs.start.pub_key = *key ;
	prov->conf_inputs.start.auth_method = *(key + 1);
	prov->conf_inputs.start.auth_size = *(key + 2);
	prov->conf_inputs.start.auth_action = (*(key + 3)) + (*(key + 4) << 8);

	l_info("%s pub_type %d auth_method %d auth_size %d auth_action %d", __func__, prov->conf_inputs.start.pub_key, prov->conf_inputs.start.auth_method, prov->conf_inputs.start.auth_size, prov->conf_inputs.start.auth_action);

	if (prov->conf_inputs.start.pub_key == 1) {
		if(prov->conf_inputs.caps.pub_type == 0) {
			l_debug("node not support public key user choose public key prov_manaual error\n");
		}
	}

	/* Parse OOB Options, prefer static, then out, then in */
	if (prov->conf_inputs.start.auth_method == 1) {
		if (prov->conf_inputs.caps.static_type == 0) {
			l_debug("user choose static oob prov_manaual error\n");
		}
	}

	if (prov->conf_inputs.start.auth_method == 2) {
		if (prov->conf_inputs.start.auth_action == 0 || prov->conf_inputs.start.auth_size == 0) {
			l_debug("user choose output oob prov_manaual error\n");
		}

	}

	if (prov->conf_inputs.start.auth_method == 3) {
		if (prov->conf_inputs.start.auth_action == 0 || prov->conf_inputs.start.auth_size == 0) {
			l_debug("user choose inputput oob prov_manaual error\n");
		}
	}

	prov->state = INT_PROV_RECV_CHOOSE_AUTH;
	//prov->state = INT_PROV_INVITE_ACKED;
	/* If Public Key available Out of Band, use it */

	if (prov->conf_inputs.caps.pub_type) {
		prov->state = INT_PROV_REQUEST_PUBLIC_KEY;
		prov->conf_inputs.start.pub_key = 0x01;
		prov->expected = PROV_CONFIRM;
		/* Prompt Agent for remote Public Key */
		l_debug("choose_caps_cb goto agent_request_public_key");
	...
	if (prov->conf_inputs.caps.pub_type == 0) {
		...
		l_info("choose_caps_cb PROV_CAPS send prov_start pub_key %d auth %d action %d size %d", prov->conf_inputs.start.pub_key, prov->conf_inputs.start.auth_method, prov->conf_inputs.start.auth_action, prov->conf_inputs.start.auth_size);
		prov->state = INT_PROV_START_SENT;
		prov->trans_tx(prov->trans_data, out,
				sizeof(prov->conf_inputs.start) + 1);
		l_free(out);
...
#ifdef CONFIG_XR829_BT
		if (prov->prov_manaual != 0) {
			prov->state = INT_PROV_REQUEST_CHOOSE_CAP;
			int_prov_start_auth(mesh_agent_get_caps(prov->agent),
							&prov->conf_inputs.caps,
							&prov->conf_inputs.start);
			mesh_agent_request_choose_caps(prov->agent, choose_caps_cb, &(prov->conf_inputs.caps));
		}

		if (prov->prov_manaual == 0) {
		XR_INITIATOR_FORCE_TO_NOOB_PATH(prov);
			prov->expected = PROV_PUB_KEY;
			...
			l_info("PROV_CAPS send prov_start pub_key %d auth %d", prov->conf_inputs.start.pub_key, prov->conf_inputs.start.auth_method);
			prov->state = INT_PROV_START_SENT;
			prov->trans_tx(prov->trans_data, out,
						sizeof(prov->conf_inputs.start) + 1);
			l_free(out);
...
#ifdef CONFIG_XR829_BT
        //fix change to send confirm value and make public key receviced fail from peer device
        l_info("%s fix not to send confirm value\n",__func__);
        if(prov->conf_inputs.start.pub_key == 1)
            int_prov_auth();
        break;
#else
		...
		void *node, void *caller_data)
{
	...
	prov->previous = -1;
	prov->prov_manaual = prov_manaual_global;
...
int32_t prov_initiator_set_manaual_choose(uint8_t manaual) {
	prov_manaual_global = manaual;
	l_info(" manaual %d", prov_manaual_global);
	return prov_manaual_global;
}
