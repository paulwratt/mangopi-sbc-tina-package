...
static void index_added(uint16_t index, uint16_t length, const void *param,
							void *user_data)
{
	bluez554_mgmt_send(mgmt_mesh, MGMT_OP_READ_INFO, index, 0, NULL,
...
static bool mesh_mgmt_init(void)
{
	...
	if (!mgmt_mesh) {
		mgmt_mesh = bluez554_mgmt_new_default();
		...
		bluez554_mgmt_register(mgmt_mesh, MGMT_EV_INDEX_ADDED,
				MGMT_INDEX_NONE, index_added, NULL, NULL);
		bluez554_mgmt_register(mgmt_mesh, MGMT_EV_INDEX_REMOVED,
...
bool mesh_mgmt_list(mesh_mgmt_read_info_func_t cb, void *user_data)
{
	...
	if (bluez554_mgmt_send(mgmt_mesh, MGMT_OP_READ_INDEX_LIST,
