...
static void forward_model(void *a, void *b)
{
	...
	if (dst == fwd->unicast || IS_FIXED_GROUP_ADDRESS(dst)) {
		...
		/*l_debug("find mod->subs dst %x", dst);*/
	...
	result = false;
    xr_info(TRC_MODEL,"call mod->cbs->recv dst %x fwd->unicast %x app_idx %d mod->id %x\n", dst, fwd->unicast, fwd->app_idx, mod->id);
#ifdef CONFIG_XR829_BT
    if (mod->cbs->recv)
		...
		const void *msg, uint16_t msg_len)
{
	...
	l_debug("(%x) %p net_idx %x key_aid %x iv_index %d seq_num %d", app_idx, key, net_idx, key_aid, iv_index, seq_num);
	xr_print_packet("key: ", key, 16);
						...
						uint16_t app_idx, bool unbind)
{
	...
	if (!mod) {
		l_debug("Model not found id %x",id);
		return status;
	}
	...
	if (!is_present && unbind) {
		l_debug("is_present is 0 unbind 1 return MESH_STATUS_SUCCESS mod->id %x mod->ele_idx %d app_idx %d", mod->id, mod->ele_idx, app_idx);
		return MESH_STATUS_SUCCESS;
	}

	if (is_present && !unbind) {
		l_debug("is_present is 1 unbind 0 return MESH_STATUS_SUCCESS mod->id %x mod->ele_idx %d app_idx %d", mod->id, mod->ele_idx, app_idx);
		return MESH_STATUS_SUCCESS;
	}
...
#endif
{
	...
	uint16_t addr,crpl,m_company_id,m_opcode,m_buf_len,n;
	uint8_t *m_buf,num_ele,rssi = 0;
	bool result = false,is_subscription;

	struct mesh_virtual *decrypt_virt = NULL;

	l_debug("iv_index %8.8x key_aid = %2.2x src = %x dst = %x ttl = %x ", iv_index, key_aid, src, dst, ttl);
	if (!dst)
		return false;
	...
	for (i = 0; i < num_ele; i++) {
		 ...
		 */
		/*l_debug("forward.has_dst %d forward.done %d forward.unicast %x", forward.has_dst, forward.done, forward.unicast);*/
		if (forward.has_dst && !forward.done) {
            ...
            if (node_get_owner(node) == NULL || node_get_app_path(node) == NULL) {
                l_info("node is not normal\n");
                goto done;
            }
            mesh_model_opcode_get(forward.data, forward.size, &opcode, &n);
            if (n == 3) {
                m_company_id = ((opcode & 0xff)<< 8) | ((opcode >> 8) & 0xff);
                m_opcode = opcode >> 16;
            } else {
                m_company_id = 0xffff;
                m_opcode = opcode;
            }
            m_buf = (uint8_t *)(forward.data + n);
            m_buf_len = forward.size - n;
            struct l_dbus_message *msg;
            struct l_dbus *dbus = dbus_get_bus();
            struct l_dbus_message_builder *builder;

            char *method_name = "GenericMessageCb";
            msg = l_dbus_message_new_method_call(dbus, node_get_owner(node),
                                    ...
                                    MESH_APPLICATION_INTERFACE,
                                        method_name);
            builder = l_dbus_message_builder_new(msg);
            l_dbus_message_builder_append_basic(builder, 'q', &m_company_id);
            l_dbus_message_builder_append_basic(builder, 'q', &m_opcode);
            l_dbus_message_builder_append_basic(builder, 'q', &src);
            l_dbus_message_builder_append_basic(builder, 'q', &dst);
            l_dbus_message_builder_append_basic(builder, 'q', &forward.app_idx);
            l_dbus_message_builder_append_basic(builder, 'q', &forward.net_idx);
            l_dbus_message_builder_append_basic(builder, 'y', &rssi);
            l_dbus_message_builder_append_basic(builder, 'y', &ttl);
            l_dbus_message_builder_append_basic(builder, 'y', &i);
            dbus_append_byte_array(builder, m_buf, m_buf_len);
            l_info("mesh_model_rx comp_id %x opcode %x src %x dst %x app_idx %x net_idx %x param.generic_msg.buf_len %d  Send \" generic msg to %s %s ", m_company_id,m_opcode,src,dst,forward.app_idx,forward.net_idx, m_buf_len, node_get_owner(node), node_get_app_path(node));
            print_packet("param.generic_msg.buf: ", m_buf, m_buf_len);
            l_dbus_message_builder_finalize(builder);
            l_dbus_message_builder_destroy(builder);
            l_dbus_send(dbus, msg);
#endif
