...
#define DEFAULT_TRANSMIT_COUNT		8
#define DEFAULT_TRANSMIT_INTERVAL	20
#else
...
#ifdef CONFIG_XR829_BT
    if(FRIEND_NO_ERROR == frnd->terminated_reason)
    {
        if(frnd->net->frn_status != FRIEND_NO_ERROR)
        {
            frnd->terminated_reason = frnd->net->frn_status;
        }
    }
    update_friendship_status_to_app(mesh_net_node_get(frnd->net), frnd->lp_addr, FRIENDSHIP_DISCONNECTED, frnd->terminated_reason);
#endif
	free_friend_internals(frnd);
	l_timeout_remove(frnd->timeout);
	l_free(frnd);
...
void mesh_net_free(struct mesh_net *net)
{
    ...
    net->frn_status = FRIEND_NET_FREE;
...
bool mesh_net_set_friend_mode(struct mesh_net *net, bool enable)
{
	...
	if (enable) {
	    ...
	    net->frn_status = FRIEND_FEATURE_DISABLE;
		l_queue_destroy(net->friends, mesh_friend_free);
		l_queue_destroy(net->negotiations, mesh_friend_free);
...
uint32_t mesh_net_get_iv_state(struct mesh_net *net)
{
	if (!net)
		return 0xffffffff;
...
bool mesh_net_get_iv_flag(struct mesh_net *net)
{
	if (!net)
		return 0xffffffff;
...
bool mesh_net_get_iv_upd_test_mode(struct mesh_net *net)
{
	if (!net)
		return 0xffffffff;

    return net->iv_upd_test_mode;
}

void mesh_net_set_iv_upd_test_mode(struct mesh_net *net,bool iv_upd_test_mode)
{
	if (!net)
		return ;

    net->iv_upd_test_mode = iv_upd_test_mode;
}
...
static bool is_us(struct mesh_net *net, uint16_t addr, bool src)
{
    ...
    {
        l_debug("addr %x friend %d",FRIENDS_ADDRESS,net->friend_enable);
...
not_for_friend:
	return XR_MESH_MODEL_RX_PATCH(net->node, szmic, seqAuth, seq, iv_index, ttl, net_idx,
						...
						const uint8_t *pkt, uint8_t len)
{
	...
	case NET_OP_FRND_REQUEST:
        l_info("NET_OP_FRND_REQUEST fiend_enable %d node_owner %p",net->friend_enable,node_get_owner(net->node));
		if ((!net->friend_enable)||(node_get_owner(net->node) == NULL))
			return false;
	...
	case NET_OP_HEARTBEAT:
		if (net->heartbeat.sub_enabled &&
            ...
            ctl_heartbeat_notify_app(net->node,src,dst,l_get_be16(pkt+1),rssi,hops,ttl);
				...
				const void *data, uint8_t size, int8_t rssi)
{
	...
	xr_info(TRC_NET,"net_src %x net_dst %x net_seq %x app:key_aid %x net_seqzero %x net_segN %x net_sego %x\n",     \
        net_src, net_dst, net_seq, net_key_id, net_seqZero, net_segN, net_segO);
	...
	xr_debug(TRC_NET,"check %08x net->key_id %x net_idx %x", cache_cookie, key_id, net_idx);
	...
	l_debug("RX: Network %04x -> %04x : TTL 0x%02x : IV : %8.8x SEQ 0x%06x net_ctl %d",
			net_src, net_dst, net_ttl, iv_index, net_seq, net_ctl);

	if (is_us(net, net_dst, false) ||
			(net_ctl && net_opcode == NET_OP_HEARTBEAT)) {

		xr_info(TRC_NET,"RX: App 0x%04x -> 0x%04x : TTL 0x%02x : SEQ 0x%06x seq0: 0x%04x",
					net_src, net_dst, net_ttl, net_seq, net_seqZero);
    ...
    {
        xr_debug(TRC_NET,"net packet not for us dst %x", net_dst);
    }
...
void update_iv_ivu_state(struct mesh_net *net, uint32_t iv_index,
								bool ivu)
{
	...
	l_info("iv_index %d ivu %d net->iv_upd_state %d net->iv_update %d\n", iv_index, ivu, net->iv_upd_state, net->iv_update);
...
#if 0//def MEST_TEST_LOG_ENABLE
    mesh_test_log("%s[netkey_id %x keyrefresh %d kr_phase %d snb_key_id %x snb_kr %d snb_ivi %x snb_ivu %d ]\n","RX SNB",    \
     subnet->idx,subnet->key_refresh,subnet->kr_phase,beacon_data->key_id,beacon_data->kr,beacon_data->ivi,beacon_data->ivu);
#endif
...
bool mesh_net_iv_index_update(struct mesh_net *net)
{
	...
	l_info("iv_upd_state = IV_UPD_UPDATING");
	mesh_net_flush_msg_queues(net);
	XR_NET_IV_INDEX_UPDATE_PATCH(net->node, net->iv_index, 0x01);
...
static bool clean_old_iv_index(void *a, void *b)
{
	...
	if (iv_index < 2)
	{
		/*xr_debug(TRC_NET,"%s ret false iv %d < 2",__func__,iv_index);*/
		return false;
	}
	...
	xr_debug(TRC_NET,"%s ret false rpe 0x%x < iv x%x",__func__,rpe->iv_index,iv_index - 1);
	return false;
}
#ifdef  CONFIG_XR829_BT
static void append_net_key_index(void *a, void *b)
{
	struct mesh_subnet *subnet = a;
	struct l_dbus_message_builder *builder = b;

	l_dbus_message_builder_append_basic(builder, 'q', &subnet->idx);
}
							...
							struct l_dbus_message_builder *builder)
{
	...
	l_queue_foreach(net->subnets, append_net_key_index, builder);
							...
							struct l_dbus_message_builder *builder)
{
	...
	l_queue_foreach(net->app_keys, appkey_append_key_indexes, builder);
	l_dbus_message_builder_leave_array(builder);
}
				...
				uint16_t crpl, uint32_t seq, uint32_t iv_index)
{
    ...
    if(rpe)
    {
        l_debug("Test Replay src: %4.4x seq: %6.6x iv: %8.8x rpe src: %4.4x rpe seq: %6.6x rpe iv: %8.8x",
                            src, seq, iv_index,rpe->src,rpe->seq,rpe->iv_index);
    }
    else
    {
        l_debug("Test Replay src: %4.4x seq: %6.6x iv: %8.8x rpe null",
                            src, seq, iv_index);
    }
	...
	/* SRC not in Replay Cache... see if there is space for it */
		...
		if (!ret)
#ifdef CONFIG_XR829_BT
			l_info("l_queue_length(net->replay_cache) %d crpl %d", l_queue_length(net->replay_cache), crpl);
			/*do nothing just upload*/
			/*return true;*/
#else
			return true;
#endif
