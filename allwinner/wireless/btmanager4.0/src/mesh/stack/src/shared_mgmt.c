...
#include "src/bluez-5.54_shared/mgmt.h"
...
static bool send_request(struct mgmt *mgmt, struct mgmt_request *request)
{
	...
	ret = bluez554_io_send(mgmt->io, &iov, 1);
	if (ret < 0) {
	...
	bluez554_queue_push_tail(mgmt->pending_list, request);
...
static bool can_write_data(struct io *io, void *user_data)
{
	...
	request = bluez554_queue_pop_head(mgmt->reply_queue);
	if (!request) {
		/* only reply commands can jump the queue */
		if (!bluez554_queue_isempty(mgmt->pending_list))
			return false;

		request = bluez554_queue_pop_head(mgmt->request_queue);
		if (!request)
			return false;
		...
		can_write = !bluez554_queue_isempty(mgmt->reply_queue);
	}
...
static void wakeup_writer(struct mgmt *mgmt)
{
	if (!bluez554_queue_isempty(mgmt->pending_list)) {
		...
		if (bluez554_queue_isempty(mgmt->reply_queue))
			return;
	}
	...
	bluez554_io_set_write_handler(mgmt->io, can_write_data, mgmt,
						write_watch_destroy);
}
					...
					uint16_t length, const void *param)
{
	...
	request = bluez554_bluez554_queue_remove_if(mgmt->pending_list,
					...
					uint16_t length, const void *param)
{
	...
	bluez554_queue_foreach(mgmt->notify_list, notify_handler, &match);

	mgmt->in_notify = false;

	if (mgmt->need_notify_cleanup) {
		bluez554_queue_remove_all(mgmt->notify_list, match_notify_removed,
							NULL, destroy_notify);
...
static bool can_read_data(struct io *io, void *user_data)
{
	...
	bluez554_mgmt_unref(mgmt);

	return true;
}

struct mgmt *bluez554_mgmt_new(int fd)
{
	struct mgmt *mgmt;
	...
	mgmt->request_queue = bluez554_queue_new();
	mgmt->reply_queue = bluez554_queue_new();
	mgmt->pending_list = bluez554_queue_new();
	mgmt->notify_list = bluez554_queue_new();

	if (!bluez554_io_set_read_handler(mgmt->io, can_read_data, mgmt, NULL)) {
		bluez554_queue_destroy(mgmt->notify_list, NULL);
		bluez554_queue_destroy(mgmt->pending_list, NULL);
		bluez554_queue_destroy(mgmt->reply_queue, NULL);
		bluez554_queue_destroy(mgmt->request_queue, NULL);
		bluez554_io_destroy(mgmt->io);
		free(mgmt->buf);
		free(mgmt);
		return NULL;
	}
	...
	return bluez554_mgmt_ref(mgmt);
}

struct mgmt *bluez554_mgmt_new_default(void)
{
	struct mgmt *mgmt;
	union {
...
struct mgmt *bluez554_mgmt_ref(struct mgmt *mgmt)
{
	if (!mgmt)
		return NULL;
...
void bluez554_mgmt_unref(struct mgmt *mgmt)
{
	if (!mgmt)
		return;
	...
	bluez554_mgmt_unregister_all(mgmt);
	bluez554_mgmt_cancel_all(mgmt);

	bluez554_queue_destroy(mgmt->reply_queue, NULL);
	bluez554_queue_destroy(mgmt->request_queue, NULL);

	bluez554_io_set_write_handler(mgmt->io, NULL, NULL, NULL);
	bluez554_io_set_read_handler(mgmt->io, NULL, NULL, NULL);
...
bool bluez554_mgmt_set_debug(struct mgmt *mgmt, mgmt_debug_func_t callback,
...
bool bluez554_mgmt_set_close_on_unref(struct mgmt *mgmt, bool do_close)
{
	if (!mgmt)
		return false;
...
unsigned int bluez554_mgmt_send(struct mgmt *mgmt, uint16_t opcode, uint16_t index,
				uint16_t length, const void *param,
				mgmt_request_func_t callback,
				void *user_data, mgmt_destroy_func_t destroy)
{
	...
	if (!bluez554_queue_push_tail(mgmt->request_queue, request)) {
		free(request->buf);
		free(request);
		return 0;
	}
...
unsigned int bluez554_mgmt_send_nowait(struct mgmt *mgmt, uint16_t opcode, uint16_t index,
				uint16_t length, const void *param,
				mgmt_request_func_t callback,
				void *user_data, mgmt_destroy_func_t destroy)
{
...
unsigned int bluez554_mgmt_reply(struct mgmt *mgmt, uint16_t opcode, uint16_t index,
				uint16_t length, const void *param,
				mgmt_request_func_t callback,
				void *user_data, mgmt_destroy_func_t destroy)
{
	...
	if (!bluez554_queue_push_tail(mgmt->reply_queue, request)) {
		free(request->buf);
		free(request);
		return 0;
	}
...
bool bluez554_mgmt_cancel(struct mgmt *mgmt, unsigned int id)
{
	struct mgmt_request *request;
	...
	request = bluez554_bluez554_queue_remove_if(mgmt->request_queue, match_request_id,
							UINT_TO_PTR(id));
	if (request)
		goto done;

	request = bluez554_bluez554_queue_remove_if(mgmt->reply_queue, match_request_id,
							UINT_TO_PTR(id));
	if (request)
		goto done;

	request = bluez554_bluez554_queue_remove_if(mgmt->pending_list, match_request_id,
							UINT_TO_PTR(id));
	if (!request)
		return false;
...
bool bluez554_mgmt_cancel_index(struct mgmt *mgmt, uint16_t index)
{
	if (!mgmt)
		return false;

	bluez554_queue_remove_all(mgmt->request_queue, match_request_index,
					UINT_TO_PTR(index), destroy_request);
	bluez554_queue_remove_all(mgmt->reply_queue, match_request_index,
					UINT_TO_PTR(index), destroy_request);
	bluez554_queue_remove_all(mgmt->pending_list, match_request_index,
					UINT_TO_PTR(index), destroy_request);

	return true;
}

bool bluez554_mgmt_cancel_all(struct mgmt *mgmt)
{
	if (!mgmt)
		return false;

	bluez554_queue_remove_all(mgmt->pending_list, NULL, NULL, destroy_request);
	bluez554_queue_remove_all(mgmt->reply_queue, NULL, NULL, destroy_request);
	bluez554_queue_remove_all(mgmt->request_queue, NULL, NULL, destroy_request);

	return true;
}

unsigned int bluez554_mgmt_register(struct mgmt *mgmt, uint16_t event, uint16_t index,
				mgmt_notify_func_t callback,
				void *user_data, mgmt_destroy_func_t destroy)
{
	...
	if (!bluez554_queue_push_tail(mgmt->notify_list, notify)) {
		free(notify);
		return 0;
	}
...
bool bluez554_mgmt_unregister(struct mgmt *mgmt, unsigned int id)
{
	struct mgmt_notify *notify;
	...
	notify = bluez554_bluez554_queue_remove_if(mgmt->notify_list, match_notify_id,
							UINT_TO_PTR(id));
	if (!notify)
		return false;
...
bool bluez554_mgmt_unregister_index(struct mgmt *mgmt, uint16_t index)
{
	if (!mgmt)
		return false;

	if (mgmt->in_notify) {
		bluez554_queue_foreach(mgmt->notify_list, mark_notify_removed,
							UINT_TO_PTR(index));
		mgmt->need_notify_cleanup = true;
	} else
		bluez554_queue_remove_all(mgmt->notify_list, match_notify_index,
					UINT_TO_PTR(index), destroy_notify);

	return true;
}

bool bluez554_mgmt_unregister_all(struct mgmt *mgmt)
{
	if (!mgmt)
		return false;
	...
	if (mgmt->in_notify) {
		...
		bluez554_queue_remove_all(mgmt->notify_list, NULL, NULL, destroy_notify);
