...
#include "src/bluez-5.54_shared/config.h"
...
void bluez554_queue_destroy(struct queue *queue, bluez554_queue_destroy_func_t destroy)
{
	...
	bluez554_queue_remove_all(queue, NULL, NULL, destroy);
				...
				void *user_data, bluez554_queue_destroy_func_t destroy)
{
	...
	if (function) {
		while (entry) {
			...
			data = bluez554_bluez554_queue_remove_if(queue, function, user_data);
