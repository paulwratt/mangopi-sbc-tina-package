							...
							uint8_t *secret)
{
	...
	bluez554_ecdh_shared_secret(tmp, priv, secret);
...
static void priv_key_cb(void *user_data, int err, uint8_t *key, uint32_t len)
{
	...
	bluez554_ecc_make_public_key(prov->private_key,
...
static void acp_prov_rx(void *user_data, const uint8_t *data, uint16_t len)
{
	...
	case PROV_START: /* Prov Start */
		...
		if (prov->conf_inputs.start.pub_key) {
			...
			bluez554_ecc_make_key(prov->conf_inputs.dev_pub_key,
					prov->private_key);
			swap_u256_bytes(prov->conf_inputs.dev_pub_key);
			swap_u256_bytes(prov->conf_inputs.dev_pub_key + 32);
			prov->material |= MAT_LOCAL_PRIVATE;
	...
	case PROV_RANDOM: /* Random Value */
		...
		mesh_crypto_aes_cmac(prov->calc_key,
					...
					prov->calc_key);
		xr_print_packet("ProverConfirm", data, 16);
		xr_print_packet("DeviceConfirm", prov->confirm, 16);
	...
	case PROV_DATA: /* Provisioning Data */
		...
		xr_print_packet("product net_key", info->net_key, 16);
		xr_debug(TRC_PROV,"Provisioning Unicast:%x net_index:%x iv_index:%x flags:%x",
			info->unicast, info->net_index, info->iv_index, info->flags);
        xr_info(TRC_PROV,"goto complt prov\n");
