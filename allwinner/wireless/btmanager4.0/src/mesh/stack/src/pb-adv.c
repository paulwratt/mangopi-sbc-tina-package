...
static uint16_t prov_data_interval = 50;
#endif
							...
							uint16_t size)
{
	...
	pb_adv_send(session, MESH_IO_TX_COUNT_UNLIMITED, prov_data_interval,
							buf, init_size + 10);
...
static void send_open_req(struct pb_adv_session *session)
{
	...
	pb_adv_send(session, MESH_IO_TX_COUNT_UNLIMITED, prov_data_interval, &open_req,
							sizeof(open_req));
}
...
static void send_open_cfm(struct pb_adv_session *session)
{
	...
	uint8_t open_beacon[1] = { MESH_AD_TYPE_BEACON };
...
#ifdef CONFIG_XR829_BT
	mesh_send_cancel(open_beacon, 1);
#else
	mesh_send_cancel(filter, sizeof(filter));
#endif

	pb_adv_send(session, MESH_IO_TX_COUNT_UNLIMITED, prov_data_interval, &open_cfm,
							sizeof(open_cfm));
}
...
static void send_ack(struct pb_adv_session *session, uint8_t trans_num)
{
	...
	pb_adv_send(session, 1, prov_data_interval, &ack, sizeof(ack));
}
...
static void send_close_ind(struct pb_adv_session *session, uint8_t reason)
{
	...
	pb_adv_send(session, 10, prov_data_interval, &close_ind, sizeof(close_ind));
}

#ifdef CONFIG_XR829_BT
uint16_t prov_set_data_resend_interval(uint16_t interval)
{
	prov_data_interval = interval;
	return prov_data_interval;
}
#endif
