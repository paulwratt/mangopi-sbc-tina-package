...
{
    xr_print_packet("prov rx:",data,len);
	if (user_data != &mesh)
		return;
...
void mesh_cleanup(void)
{
	...
	xr_info(TRC_PROV, "mesh_cleanup clean node model MESH_NETWORK_INTERFACE\n");
							...
							struct mesh_node *node)
{
	...
	if (status != MESH_ERROR_NONE) {
        xr_error(TRC_PROV,"%s Create Node errcode 0x%x\n",__func__,status);
						...
						void *user_data)
{
	...
	xr_print_packet("import_call devicekey: ", dev_key, 16);
	xr_print_packet("import_call netkey: ", net_key, 16);
...
#ifdef CONFIG_XR829_BT
void build_prov_db_reply(struct l_dbus_message *reply)
{
	struct l_dbus_message_builder *builder;
	struct l_queue *nodes;

	nodes = node_get_all_nodes();

	builder = l_dbus_message_builder_new(reply);
    xr_info(TRC_PROV,"%s,%d",__FUNCTION__,l_queue_length(nodes));
	/* Array of node configurations "a*/
	l_dbus_message_builder_enter_array(builder, "(yqtayayaqa(qq)a(ya(qa{sv})))");
	l_queue_foreach(nodes, node_build_config, builder);
	l_dbus_message_builder_leave_array(builder);
	l_dbus_message_builder_finalize(builder);
	l_dbus_message_builder_destroy(builder);
}

static struct l_dbus_message *get_prov_db_call(struct l_dbus *dbus,
						struct l_dbus_message *msg,
						void *user_data)
{
	struct l_dbus_message *reply;
	l_debug("Get provisioning database request");
	reply = l_dbus_message_new_method_return(msg);
	build_prov_db_reply(reply);
...
void update_iv_to_app(struct mesh_node *node, uint32_t iv_index, uint8_t flags)
{
	...
	if (node == NULL) {
		l_info("update_iv_to_app node is null\n");
		return;
	}
	char *method_name = "IvUpdateState";/*mi_mesh_event_to_dbus_method(MIBLE_MESH_EVENT_GENERIC_MESSAGE_CB);*/
	...
	l_debug("update_iv_to_app Send \" iv update msg \"  to %s %s",
									node_get_owner(node), node_get_app_path(node));
	l_dbus_send(dbus, msg);
	return;
}

void update_friendship_status_to_app(struct mesh_node *node, uint16_t src , uint8_t status, uint8_t reason)
{
	struct l_dbus_message *msg;
	struct l_dbus *dbus = dbus_get_bus();
	struct l_dbus_message_builder *builder;
	...
	char *method_name = "FriendshipState";
	msg = l_dbus_message_new_method_call(dbus,node_get_owner(node),
									node_get_app_path(node),
									MESH_APPLICATION_INTERFACE,
								method_name);
    ...
    l_dbus_message_builder_append_basic(builder, 'y', &reason);
    l_dbus_message_builder_finalize(builder);
	l_dbus_message_builder_destroy(builder);
	l_debug("update_friendship_status_to_app Send \" friendship status msg \"	to %s %s",
									node_get_owner(node), node_get_app_path(node));
	l_dbus_send(dbus, msg);
	return;
}

void mesh_set_scan_enable(bool enable)
{
	set_scan_enable(mesh.io, enable);
}
...
int32_t beacon_num = 0;
static struct l_dbus_message *send_beacon(struct l_dbus *dbus,
						struct l_dbus_message *msg,
						void *user_data)
{
	const char *app_path;
	...
	if (!l_dbus_message_get_arguments(msg, "oqq", &app_path, &num, &interval))
		return dbus_error(msg, MESH_ERROR_INVALID_ARGS, NULL);

	l_debug("send_beacon num %d interval %d\n", num, interval);
	/*modify for adv beacon test*/
	...
	l_put_be16(0x0, beacon + 18);
	mesh_send_pkt(0, interval, beacon, len);
	beacon_num = num + 2;
...
uint16_t mesh_net_set_crpl(uint16_t crpl)
{
	mesh.crpl = crpl;
	return mesh.crpl;
}
...
static void setup_network_interface(struct l_dbus_interface *iface)
{
	...
	l_dbus_interface_method(iface, "Import", 0,
					...
					"unicast");
#ifdef CONFIG_XR829_BT
	l_dbus_interface_method(iface, "GetProvDb", 0, get_prov_db_call,
				"a(yqtayayaqa(qq)a(ya(qa{sv})))", "", "ttl", "unicast", "dev_key",
				"net_keys", "app_keys", "configuration");
	l_dbus_interface_method(iface, "ScanEnable", 0, scan_enable, "",
				"ob", "app", "enable");
	l_dbus_interface_method(iface, "SendBeacon", 0, send_beacon, "",
				"oqq", "app", "num", "interval");
#endif
}
