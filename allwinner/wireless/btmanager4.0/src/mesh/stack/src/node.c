...
{
	if (!node) {
		xr_info(TRC_PROV, "free_node_dbus_resources node is null\n");
		return;
	}
	...
	if (node->obj_path) {
		xr_info(TRC_PROV, "free_node_dbus_resources free MESH_NODE_INTERFACE MESH_MANAGEMENT_INTERFACE\n");
...
static void free_node_resources(void *data)
{
	...
	l_info("free_node_resources %s ", node->owner);
...
void node_remove(struct mesh_node *node)
{
	...
	l_info("node_remove and free node\n");
					...
					struct mesh_config_node *db_node)
{
	...
	XR_ADD_INTERNAL_MODEL_PATCH(node);
...
{
	xr_info(TRC_PROV, "node_cleanup_all unregister MESH_NODE_INTERFACE MESH_MANAGEMENT_INTERFACE\n");
	...
	for (; ele_entry; ele_entry = ele_entry->next) {
		...
		for (; mod_entry; mod_entry = mod_entry->next) {
			...
			if (vendor != 0xffff) {
				...
				n += 2;
				num_v++;
				l_info("vendor %x mod_id %x", vendor, mod_id);
			}

		}
	l_info("node_generate_comp num_s %d num_v %d", num_s, num_v);
element_done:
		mod_buf[0] = num_s;
		mod_buf[1] = num_v;
...
static bool register_node_object(struct mesh_node *node)
{
	...
	if (!hex2str(node->uuid, sizeof(node->uuid), uuid, sizeof(uuid))) {
		xr_info(TRC_PROV, "register_node_object hex2str fail\n");
		return false;
	}
	...
	xr_info(TRC_PROV, "register_nod_object begin path %s\n", node->obj_path);
	if (!l_dbus_object_add_interface(dbus_get_bus(), node->obj_path,
						MESH_NODE_INTERFACE, node)) {
		xr_info(TRC_PROV, "register_node_object add MESH_NODE_INTERFACE fail\n");
		return false;
	}

	if (!l_dbus_object_add_interface(dbus_get_bus(), node->obj_path,
					MESH_MANAGEMENT_INTERFACE, node)) {
		xr_info(TRC_PROV, "register_node_object add MESH_MANAGEMENT_INTERFACE fail\n");
		return false;
	}

	if (!l_dbus_object_add_interface(dbus_get_bus(), node->obj_path,
					L_DBUS_INTERFACE_PROPERTIES, NULL)) {
		xr_info(TRC_PROV, "register_node_object add L_DBUS_INTERFACE_PROPERTIES fail\n");
		return false;
	}

	xr_info(TRC_PROV,"register_node_object node->obj_paths %s", node->obj_path);
	return true;
}
...
static void app_disc_cb(struct l_dbus *bus, void *user_data)
{
	...
	l_info("app_disc_cb App %s disconnected (%u)", node->owner, node->disc_watch);
					...
					struct l_dbus_message_iter *property)
{
	...
	while (l_dbus_message_iter_next_entry(&ids, &mod_id)) {
		...
		l_info("get_sig_models_from_properties id is %x", id);
		l_queue_insert(ele->models, mod, compare_model_id, NULL);
	}
					...
					struct l_dbus_message_iter *property)
{
	...
	while (l_dbus_message_iter_next_entry(&ids, &vendor_id, &mod_id)) {
		...
		/* Disallow duplicates */
		l_info("get_vendor_models_from_properties id %x", id);
		if (l_queue_find(ele->models, match_model_id,
							L_UINT_TO_PTR(id)))
			return ;

		mod = mesh_model_new(ele->idx, id);
		l_info("get_vendor_models_from_properties id is %x", id);
		l_queue_insert(ele->models, mod, compare_model_id, NULL);
	}
...
static bool xr_get_sig_models_from_properties(struct node_element *ele,
					struct l_dbus_message_iter *property)
{
    get_sig_models_from_properties(ele,property);
    return true;
}

static bool xr_get_vendor_models_from_properties(struct node_element *ele,
					struct l_dbus_message_iter *property)
{
    get_vendor_models_from_properties(ele,property);
    return true;
}
#else
					...
					struct l_dbus_message_iter *properties)
{
	...
	while (l_dbus_message_iter_next_entry(properties, &key, &var)) {
		if (!strcmp(key, "Index")) {
			...
			if (mods || !XR_GET_SIG_MODELS_FROM_PROP(ele, &var))
				xr_goto_fail();

			mods = true;
		} else if (!strcmp(key, "VendorModels")) {
			if (vendor_mods ||
				!XR_GET_VENDOR_MODELS_FROM_PROP(ele, &var))
				xr_goto_fail();
					...
					struct mesh_config_node *db_node)
{
	...
	l_debug("node ele num %d", l_queue_length(node->elements));
	for (; entry; entry = entry->next) {
		struct node_element *ele = entry->data;
		struct mesh_config_element *db_ele;
		const struct l_queue_entry *mod_entry;
					...
					struct l_dbus_message_iter *properties)
{
    ...
    if(feature)
    {
        if((node->beacon == MESH_MODE_UNSUPPORTED)&&(feature & AW_MESH_FEATURE_SNB))
        {
            node->beacon = MESH_MODE_DISABLED;
        }
        if((node->friend== MESH_MODE_UNSUPPORTED)&&(feature & AW_MESH_FEATURE_FRIEND))
        {
            node->friend = MESH_MODE_DISABLED;
        }
        if((node->proxy == MESH_MODE_UNSUPPORTED)&&(feature & AW_MESH_FEATURE_PROXY))
        {
            node->beacon = MESH_MODE_DISABLED;
        }
        if((node->lpn == MESH_MODE_UNSUPPORTED)&&(feature & AW_MESH_FEATURE_LPN))
        {
            node->lpn = MESH_MODE_DISABLED;
        }
        if((node->relay.mode == MESH_MODE_UNSUPPORTED)&&(feature & AW_MESH_FEATURE_RELAY))
        {
            node->relay.mode = MESH_MODE_DISABLED;
        }
        l_debug("mesh stack get feature form app is %x ",feature);
    }
...
static void update_element_config(struct mesh_node *node, struct mesh_node *attach)
{
	...
	convert_node_to_storage(node, &db_node);
	update_element_config_jscon(attach->cfg, &db_node);

	l_debug("attach ele num %d node ele num %d", l_queue_length(attach->elements), l_queue_length(node->elements));
	if (!mesh_config_save(attach->cfg, true, NULL, NULL)) {
		mesh_config_release(attach->cfg);
		l_debug("update_element_config mesh_config_save fail and release\n");
		return;
	}
...
static void add_node_element(struct mesh_node *node, struct mesh_node *attach, struct node_element *ele)
{
	struct node_element *ele_tmp = l_new(struct node_element, 1);
	struct mesh_config_node db_node;
	const struct l_queue_entry *entry;
	if(!ele_tmp){
		l_debug("update_element ele_tmp null\n");
		return;
	}

	l_debug("add ele num %d", ele->idx);
	ele_tmp->location = ele->location;
	ele_tmp->idx = ele->idx;
	ele_tmp->path = l_strdup(ele->path);
	if (!ele->models)
		ele->models = l_queue_new();
	...
	{
		struct mesh_config_element *db_ele = entry->data;
		if(db_ele->index == ele_tmp->idx)
		{
			add_models_from_storage(attach, ele_tmp, db_ele);
			break;
		}
	}

	l_queue_insert(attach->elements, ele_tmp, compare_element_idx, NULL);
	entry = l_queue_get_entries(db_node.elements);
	for (; entry; entry = entry->next) {
		struct mesh_config_element *db_ele = entry->data;
		l_queue_destroy(db_ele->models, l_free);
...
static void update_element(struct mesh_node *node, struct mesh_node *attach)
{
	uint32_t len_node, len_attach, len, i;
	struct node_element *ele, *ele_attach;

	len_node = l_queue_length(node->elements);
	len_attach = l_queue_length(attach->elements);

	if (len_node >= len_attach)
		len = len_node;
	else
		len = len_attach;
	l_debug("node ele num %d  attach ele num %d", len_node, len_attach);
	for (i = 0; i < len; i++) {
		...
		if (!ele && !ele_attach){
			continue;
		} else if (!ele && ele_attach != NULL) {
			l_debug("remove attach ele num %d\n", i);
			l_queue_remove(attach->elements, ele_attach);
			element_free(ele_attach);
		} else if (ele != NULL && !ele_attach) {
			l_debug("add ele to attach ele num %d\n", i);
			add_node_element(node, attach, ele);
		} else {
			l_debug("update ele_attach ele num %d\n", i);
			l_queue_remove(attach->elements, ele_attach);
			element_free(ele_attach);
			add_node_element(node, attach, ele);
		}
	}
	XR_CFGMOD_INIT_PATCH(attach);
	update_element_config(node, attach);
}

static void copy_node_info_to_attach(struct managed_obj_request *req)
{
	update_composition(req->node, req->attach);
	update_element(req->node, req->attach);
}
#endif
static bool check_req_node(struct managed_obj_request *req)
{
	...
	if (!node_len) {
		l_info("check_req_node node_len is 0\n");
		return false;
	}
	...
	if (req->type == REQUEST_TYPE_ATTACH) {
		...
		if (node_len != attach_len ||
			...
			l_debug("check_req_node not match just update attach info according node info\n");
			copy_node_info_to_attach(req);
		}
#else
		if (node_len != attach_len ||
				memcmp(node_comp, attach_comp, node_len)) {
...
static bool attach_req_node(struct mesh_node *attach, struct mesh_node *node)
{
	...
	if (!register_node_object(attach)) {
		xr_info(TRC_PROV, "register_nod_object fail and attach free_node_dbus_resources\n");
	...
	node->owner = NULL;
	xr_info(TRC_PROV,"attach_req_node and node_remove\n");
	node_remove(node);
...
static void get_managed_objects_cb(struct l_dbus_message *msg, void *user_data)
{
	...
	while (l_dbus_message_iter_next_entry(&objects, &path, &interfaces)) {
		...
		while (l_dbus_message_iter_next_entry(&interfaces, &interface,
			...
			if (!strcmp(MESH_ELEMENT_INTERFACE, interface)) {
				l_info("get_managed_objiects_cb get_element_properties");
	...
	l_info("get_managed_objects_cb goto check_req_node node->num_ele %d", node->num_ele);
...
fail:
    xr_error(TRC_MGR,"%s MESH_ERROR_FAILED\n",__func__);
	if (agent)
...
{
	struct managed_obj_request *req;
	int32_t status = MESH_STATUS_SUCCESS;

	l_debug("");
#ifdef CONFIG_XR829_BT
	struct mesh_node *f_node = NULL;
	f_node = node_find_by_uuid((uint8_t *)uuid);
	if (f_node != NULL) {
		l_info("node_import f_node is not null");

		if (!mesh_config_write_iv_index(f_node->cfg, iv_index, ivu))
			return false;

		update_iv_ivu_state(f_node->net, iv_index, ivu);
		mesh_net_set_iv_index(f_node->net, iv_index, ivu);

		if (!mesh_config_write_unicast(f_node->cfg, unicast))
			return false;

		memcpy(f_node->dev_key, dev_key, 16);
		if (!mesh_config_write_device_key(f_node->cfg, (uint8_t *)dev_key))
			return false;

		f_node->primary = unicast;
		mesh_net_register_unicast(f_node->net, unicast, f_node->num_ele);

		status = mesh_net_add_key(f_node->net, net_idx, net_key);
		if (status == MESH_STATUS_IDX_ALREADY_STORED)
		{
			l_debug("net_add key %d fail already stored", net_idx);
			status = mesh_net_del_key(f_node->net, net_idx);
			if (status != MESH_STATUS_SUCCESS) {
				l_info("net_add key already restore fail status %d", status);
			}
			status = mesh_net_add_key(f_node->net, net_idx, net_key);
			if (status != MESH_STATUS_SUCCESS) {
				l_debug("net_add key fail status %d", status);
			}
		} else if (status != MESH_STATUS_SUCCESS) {
			l_info("add netkey fail status %d", status);
			return false;
		}

		if (kr) {
			/* Duplicate net key, if the key refresh is on */
			if (mesh_net_update_key(f_node->net, net_idx, net_key) !=
								MESH_STATUS_SUCCESS)
				return false;

			if (!mesh_config_net_key_set_phase(f_node->cfg, net_idx,
								KEY_REFRESH_PHASE_TWO))
				return false;
		}
		mesh_config_save(f_node->cfg, true, NULL, NULL);
		/* Initialize configuration server model */
        XR_CFGMOD_INIT_PATCH(f_node);

		cb(user_data, MESH_ERROR_NONE, f_node);
		return true;
	}
#endif
	req = l_new(struct managed_obj_request, 1);
						...
						void *user_data)
{
	...
	if (app_idx & ~APP_IDX_MASK)
		return dbus_error(msg, MESH_ERROR_INVALID_ARGS,
						"Invalid key_index");
    xr_debug(TRC_MGR,"Dst: 0x%x, App_idx: %d, Access payload: %d bytes src 0x%x", dst, app_idx, len, src);
    xr_print_packet("send data:",data,len);
						...
						void *user_data)
{
	...
	xr_info(TRC_MGR,"dev_key_send_call src %x dst %x net_idx %x\n", src, dst, net_idx);
	xr_print_packet("data: ", data, len);

#ifdef CONFIG_XR829_BT
	if(dst == 0)
    {
		dst = src;
		if (!mesh_config_cli_msg_handler(node, src, dst,net_idx,
			mesh_net_get_default_ttl(node->net), data, len))
        {
		l_debug("mesh_config_cli_msg handler error");
			//return dbus_error(msg, MESH_ERROR_FAILED, NULL);
        }
		return l_dbus_message_new_method_return(msg);
    }
...
#ifdef CONFIG_XR829_BT
static struct l_dbus_message *get_iv_info(struct l_dbus *dbus,
						struct l_dbus_message *msg,
						void *user_data)
{
	struct mesh_node *node = user_data;
	...
	l_dbus_message_set_arguments(reply, "uyy", mesh_net_get_iv_index(net), mesh_net_get_iv_state(net),mesh_net_get_iv_flag(net));
	return reply;
}

static struct l_dbus_message *iv_test_mode(struct l_dbus *dbus,
						struct l_dbus_message *msg,
						void *user_data)
{
	struct mesh_node *node = user_data;
	...
	if (!l_dbus_message_get_arguments(msg, "y",&test_mode))
		return dbus_error(msg, MESH_ERROR_INVALID_ARGS, NULL);

    l_debug("%s: iv test_mode %d",__func__,test_mode);
    mesh_net_set_iv_upd_test_mode(net,test_mode);

	reply = l_dbus_message_new_method_return(msg);
	l_dbus_message_set_arguments(reply, "y", mesh_net_get_iv_upd_test_mode(net));
	return reply;
}

static struct l_dbus_message *update_iv_info(struct l_dbus *dbus,
						struct l_dbus_message *msg,
						void *user_data)
{
	struct mesh_node *node = user_data;
	...
	uint32_t iv_index;
	uint8_t flags;
	bool ivu,test_mode = false;

	l_debug("update_iv_info");
	...
	if (!l_dbus_message_get_arguments(msg, "uy",
							&iv_index, &flags))
		return dbus_error(msg, MESH_ERROR_INVALID_ARGS, NULL);

    test_mode = mesh_net_get_iv_upd_test_mode(net);

	l_debug("iv_index %d flags %d test_mode %d", iv_index, flags,test_mode);

	ivu = flags;

    mesh_net_set_iv_upd_test_mode(net,true);
	update_iv_ivu_state(net, iv_index, ivu);
    mesh_net_set_iv_upd_test_mode(net,test_mode);
...
static struct l_dbus_message *set_manaual_choose(struct l_dbus *dbus,
						struct l_dbus_message *msg,
						void *user_data)
{
	struct mesh_node *node = user_data;
	...
	if (!l_dbus_message_get_arguments(msg, "y", &manaual))
		return dbus_error(msg, MESH_ERROR_INVALID_ARGS, NULL);

	l_debug(" prov_initiator_set_manaual_choose");
	prov_initiator_set_manaual_choose(manaual);

	reply = l_dbus_message_new_method_return(msg);
	return reply;
}

static struct l_dbus_message *set_mesh_protocol_param(struct l_dbus *dbus,
						struct l_dbus_message *msg,
						void *user_data)
{
	struct mesh_node *node = user_data;
	...
	if (!l_dbus_message_get_arguments(msg, "qqi", &crpl, &prov_inter, &feature))
		return dbus_error(msg, MESH_ERROR_INVALID_ARGS, NULL);

	l_debug("crpl %d prov_inter %d feature %d", crpl, prov_inter, feature);
	node->comp.crpl = crpl;
	if (!mesh_config_update_crpl(node->cfg, node->comp.crpl)) {
		l_debug("save crpl fail");
	}
	mesh_net_set_crpl(crpl);
	prov_set_data_resend_interval(prov_inter);
...
#ifdef CONFIG_XR829_BT
	l_dbus_interface_method(iface, "IvTestMode", 0, iv_test_mode, "","y", "test_mode");
	l_dbus_interface_method(iface, "GetIvInfo", 0, get_iv_info, "","");
	l_dbus_interface_method(iface, "UpdateIvInfo", 0, update_iv_info, "",
				"uy", "index", "flags");
	l_dbus_interface_method(iface, "SetManaualChoose", 0, set_manaual_choose, "",
				"y", "enable");

	l_dbus_interface_method(iface, "SetMeshProtocolParam", 0, set_mesh_protocol_param, "",
				"qqi", "crpl", "prov_data_resend_interval", "feature");
...
{
	l_info("node_dbus_init resigter MESH_NODE_INTERFACE\n");
...
void node_build_config(void *a, void *b)
{
	...
	struct l_dbus_message_builder *builder = b;
    const uint8_t *token;
    l_info("build node cfg\n");
	/* unicast 0x0001 is the local node */
	if (node->primary == 0x0001)
    {
        l_info("get local node information\n");
		//return;
    }
	l_info("Node 0x%x", node->primary);

	l_dbus_message_builder_enter_struct(builder, "yqtayayaqa(qq)a(ya(qa{sv}))");

	/* append ttl */
	l_dbus_message_builder_append_basic(builder, 'y', &node->ttl);

	/* append unicast addr */
	l_dbus_message_builder_append_basic(builder, 'q', &node->primary);
	token = node_get_token(node);
    //l_info("token[]=%x,%x,%x,%x,%x,%x,%x,%x",token[0],token[1],token[2],token[3],token[4],token[5],token[6],token[7]);
    /* append token */
    l_dbus_message_builder_append_basic(builder, 't', token);

    dbus_append_byte_array(builder, node->uuid, 16);

	/* append dev key */
	dbus_append_byte_array(builder, node->dev_key, 16);

	/* append net key indexes */
	mesh_net_build_net_key_indexes(net, builder);

	/* append app key indexes */
	mesh_net_build_app_key_indexes(net, builder);
...
static char *features_string(uint8_t mode)
{
	switch (mode) {
...
{
	struct node_composition *comp = &node->comp;

	/* print composition */
	l_debug("Composition:");
	l_debug("cid:%04x pid:%04x vid:%04x crpl:%04x", comp->cid, comp->pid,
						comp->vid, comp->crpl);

	/*  print features */
	l_debug("Features:");
	l_debug("relay:%s lowPower:%s friend:%s proxy:%s", features_string(node->relay.mode),
							features_string(node->lpn),
							features_string(node->friend),
							features_string(node->proxy));

	/* TODO: print elements */
	l_debug("app_path: %s", node->app_path);
	l_debug("obj_path: %s", node->obj_path);
	l_debug("owner: %s", node->owner);
}

void ctl_heartbeat_notify_app(struct mesh_node *node, uint16_t src, uint16_t dst, uint16_t feature, int8_t rssi, uint8_t hops, uint8_t ttl)
{
    struct l_dbus_message *msg;
    struct l_dbus *dbus = dbus_get_bus();
    struct l_dbus_message_builder *builder;

    char *method_name = "HearbeatPkt";
    ...
    l_dbus_message_builder_append_basic(builder, 'q', &feature);
    l_dbus_message_builder_append_basic(builder, 'y', &rssi);
    l_dbus_message_builder_append_basic(builder, 'y', &hops);
