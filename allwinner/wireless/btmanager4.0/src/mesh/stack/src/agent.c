...
typedef enum {
	...
	MESH_AGENT_REQUEST_CHOOSE_CAPS
				...
				struct l_dbus_message_iter *property)
{
	...
	l_debug("agent parse_prov_caps out_put action %d input action %d output size %d input size %d static_type %d ", caps->output_action, caps->input_action, caps->output_size, caps->input_size, caps->static_type);
}
...
static void numeric_reply(struct l_dbus_message *reply, void *user_data)
{
	...
	l_dbus_message_unref(req->msg);
	l_debug("numeric_reply err %d count %d", err, count);
	if (req->cb) {
		cb = req->cb;
...
static void choose_caps_reply(struct l_dbus_message *reply, void *user_data)
{
	struct mesh_agent *agent = user_data;
	struct agent_request *req;
	mesh_agent_choose_caps_cb_t cb;
	uint32_t n = 0;
	...
	if (!l_dbus_message_get_arguments(reply, "yyyq", &pub_type, &auth, &auth_size, &auth_action)) {
		l_error("Failed to retrieve choose caps");
		err = MESH_ERROR_FAILED;
		goto done;
	}
	...
	buf[1] = auth;
	buf[2] = auth_size;
	buf[3] = auth_action & 0xff;
	buf[4] = (auth_action & 0xff00) >> 8;
	n = 5;
...
static int request_choose_caps(struct mesh_agent *agent,
					agent_request_type_t type,
					void *cb, void *user_data)
{
	struct l_dbus *dbus = dbus_get_bus();
	...
	caps = (struct mesh_net_prov_caps *)user_data;
	agent->req = create_request(type, cb, NULL);
	method_name = "ChooseCaps";
	msg = l_dbus_message_new_method_call(dbus, agent->owner,
						agent->path,
						MESH_PROVISION_AGENT_INTERFACE,
						method_name);
	...
	builder = l_dbus_message_builder_new(msg);
	l_dbus_message_builder_append_basic(builder, 'y', &(caps->num_ele));
	l_dbus_message_builder_append_basic(builder, 'q', &(caps->algorithms));
	l_dbus_message_builder_append_basic(builder, 'y', &(caps->pub_type));
	l_dbus_message_builder_append_basic(builder, 'y', &(caps->static_type));
	l_dbus_message_builder_append_basic(builder, 'y', &(caps->output_size));
	l_dbus_message_builder_append_basic(builder, 'q', &(caps->output_action));
	l_dbus_message_builder_append_basic(builder, 'y', &(caps->input_size));
	l_dbus_message_builder_append_basic(builder, 'q', &(caps->input_action));
	l_dbus_message_builder_finalize(builder);
	l_dbus_message_builder_destroy(builder);

	l_debug("Send choosecaps request to %s %s", agent->owner, agent->path);
	l_dbus_send_with_reply(dbus_get_bus(), msg, choose_caps_reply, agent, NULL);
...
#ifdef CONFIG_XR829_BT
int mesh_agent_request_choose_caps(struct mesh_agent *agent,
				mesh_agent_choose_caps_cb_t cb, void *user_data)
{
	return request_choose_caps(agent, MESH_AGENT_REQUEST_CHOOSE_CAPS, cb, user_data);
}
#endif
