...
static void friend_poll_timeout(struct l_timeout *timeout, void *user_data)
{
    ...
    if(frnd->terminated_reason != FRIEND_CLEAR)
    {
        frnd->terminated_reason = FRIEND_POLLTIMEOUT;
    }
#endif
				...
				uint16_t lpnCounter, struct mesh_friend *frnd)
{
	...
	if (frnd) {
        ...
        update_friendship_status_to_app(mesh_net_node_get(frnd->net), frnd->lp_addr, FRIENDSHIP_DISCONNECTED, FRIEND_CLEAR);
		if (removed) {
			...
			if (old)
            {
                old->terminated_reason = FRIEND_POLLTIMEOUT;
...
#ifdef CONFIG_XR829_BT
		update_friendship_status_to_app(mesh_net_node_get(net), src, FRIENDSHIP_CONNECTED, FRIEND_NO_ERROR);
        frnd->terminated_reason = FRIEND_NO_ERROR;
#endif
