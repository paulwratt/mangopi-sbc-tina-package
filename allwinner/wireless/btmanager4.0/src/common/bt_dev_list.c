                             ...
                             unsigned int profile)
{
    ...
    if (!dev_list) {
        BTMG_DEBUG("%s: dev_list is null", __func__);
    ...
    if (dev_node == NULL) {
        BTMG_ERROR("%s: calloc for dev_node failed", __func__);
    ...
    BTMG_DEBUG("add dev node addr: %s, name: %s, profile：%d", dev_node->dev_addr,
               dev_node->dev_name, dev_node->profile);
    dev_node->next = NULL;
    pthread_mutex_lock(&dev_list->lock);

    if (dev_list->list_cleared) {
        BTMG_DEBUG("dev_list is cleared, nothing could be done");
...
dev_node_t *btmg_dev_list_find_device(dev_list_t *dev_list, const char *addr)
{
    ...
    if (addr == NULL) {
        BTMG_DEBUG("address is null.");
        return NULL;
    }
    BTMG_DUMP("address: %s", addr);
    if (dev_list == NULL) {
    ...
    if (dev_list->list_cleared) {
        BTMG_INFO("dev_list is cleared, nothing could be done");
        pthread_mutex_unlock(&dev_list->lock);
        return NULL;
    }
...
static dev_node_t *btmg_dev_list_find_device_inner(dev_list_t *dev_list, const char *addr)
{
    ...
    if (!dev_list) {
        BTMG_ERROR("%s: dev_list is null", __func__);
        return NULL;
    }

    dev_node = dev_list->head;
    if (dev_node == NULL) {
        BTMG_ERROR("%s: device node is null", __func__);
        return NULL;
    }
...
bool btmg_dev_list_remove_device(dev_list_t *dev_list, const char *addr)
{
    ...
    if (dev_node == NULL) {
        BTMG_DEBUG("device is not found in device list");
        pthread_mutex_unlock(&dev_list->lock);
        return true;
    }
