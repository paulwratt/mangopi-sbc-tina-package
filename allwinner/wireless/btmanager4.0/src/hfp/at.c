...
char *at_parse(const char *str, struct bt_at *at)
{
    ...
    BTMG_INFO("AT message: %s: command: %s, value: %s", at_type2str(at->type), at->command,
              at->value);
...
int at_parse_cind(const char *str, enum hfp_ind map[20])
{
    static const struct {
        ...
        { "service", HFP_IND_SERVICE },     { "call", HFP_IND_CALL },
        { "callsetup", HFP_IND_CALLSETUP }, { "callheld", HFP_IND_CALLHELD },
        { "signal", HFP_IND_SIGNAL },       { "roam", HFP_IND_ROAM },
...
const char *at_type2str(enum bt_at_type type)
{
    static const char *types[__AT_TYPE_MAX] = {
        [AT_TYPE_RAW] = "RAW",     [AT_TYPE_CMD] = "CMD",       [AT_TYPE_CMD_GET] = "GET",
        [AT_TYPE_CMD_SET] = "SET", [AT_TYPE_CMD_TEST] = "TEST", [AT_TYPE_RESP] = "RESP",
    };
