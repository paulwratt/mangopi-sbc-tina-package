    ...
    uint8_t *buf, int in_count, int *out_count)
{
    ...
    *out_count = write(esp_hosted_driver_handle->file_desc, buf, in_count);
    ...
    if (ioctl(esp_hosted_driver_handle->file_desc, IOCTL_ESP_FLUSH_RB, 0)) {
	    perror("ioctl IOCTL_ESP_FLUSH_RB failed\n");
    }
    return SUCCESS;
}
    ...
    int read_len, uint8_t wait, uint32_t *buf_len)
{
    ...
    do {
        ...
        timeout.tv_sec = wait;
        timeout.tv_usec = 0;
		printf("%s, wait for data to arrive on serial interface\n", __func__);
 ...
 */
            if (FD_ISSET(esp_hosted_driver_handle->file_desc, &set)) {
                check = false;
                total_read_len = 0;
				printf("%s, read request for fixed length tlv structure %d \n", __func__, read_len);
                do {
					...
					printf("%s, read bytes from fixed length tlv structure %d \n", __func__, total_read_len);
 ...
 */
                ...
                total_read_len = 0;
				printf("%s, read request for variable length tlv structure %d \n", __func__, *buf_len);
                do {
                    count = read(esp_hosted_driver_handle->file_desc,
                            (buf+total_read_len), (*buf_len-total_read_len));
                    if (count <= 0) {
                        perror("read: Failed to read vaiable length serial data");
						break;
                    }
                    total_read_len += count;
               } while (total_read_len < *buf_len);
				printf("%s, read bytes from variable length tlv structure %d \n", __func__, total_read_len);
