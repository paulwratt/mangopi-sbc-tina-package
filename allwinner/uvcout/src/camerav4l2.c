...
int setformat(camera_hal *camera)
{
	...
	if (camera->driver_type == V4L2_CAP_VIDEO_CAPTURE_MPLANE) {
		if (camera->width != fmt.fmt.pix_mp.width || camera->height != fmt.fmt.pix_mp.height)
			camerav4l2_err(" does not support %d * %d\n", camera->width, camera->height);
...
int getgain(camera_hal *camera)
{
	...
	if (-1 == ioctl (camera->videofd, VIDIOC_G_CTRL, &control)) {
		printf("VIDIOC_G_CTRL(V4L2_CID_GAIN) error!\n");
		return -1;
	}
 ...
 int setgain(camera_hal *camera, int value)
{
	struct v4l2_control control;
	...
	if (-1 == ioctl (camera->videofd, VIDIOC_S_CTRL, &control)) {
		printf("VIDIOC_S_CTRL(V4L2_CID_GAIN) error!\n");
		return -1;
	}
...
int getexposure(camera_hal *camera)
{
	struct v4l2_control control;
	...
	if (-1 == ioctl (camera->videofd, VIDIOC_G_CTRL, &control)) {
		printf("VIDIOC_G_CTRL(V4L2_CID_EXPOSURE) error!\n");
		return -1;
	}
...
int setexposure(camera_hal *camera, int value)
{
	struct v4l2_control control;
	...
	if (-1 == ioctl (camera->videofd, VIDIOC_S_CTRL, &control)) {
		printf("VIDIOC_S_CTRL(V4L2_CID_EXPOSURE) error!\n");
		return -1;
	}
 ...
 int setexposuremode(camera_hal *camera, int mode)
{
    int ret = -1;
    struct v4l2_control ctrl;
    ...
    ctrl.value = mode;
    ret = ioctl(camera->videofd, VIDIOC_S_CTRL, &ctrl);
    if (ret < 0)
        printf("setexposuremode failed, %s", strerror(errno));

    return ret;
}
 int setgainmode(camera_hal *camera, int mode)
{
    int ret = -1;
    struct v4l2_control ctrl;
    ...
    ret = ioctl(camera->videofd, VIDIOC_S_CTRL, &ctrl);
    if (ret < 0)
        printf("setgainmode failed, %s", strerror(errno));

    return ret;
}

void getexposuregain(camera_hal *camera, int *exp_val, int *gain_val)
{
    *exp_val = getexposure(camera);
    *gain_val = getgain(camera);
}

void setexposuregain(camera_hal *camera, int exp_val, int gain_val)
{
    setexposuremode(camera, V4L2_EXPOSURE_AUTO);
    setgainmode(camera, 1);

    setexposuremode(camera, V4L2_EXPOSURE_MANUAL);
    setgainmode(camera, 0);

    setexposure(camera, exp_val);
    setgain(camera, gain_val);
}

void setautoexposuregain(camera_hal *camera)
{
    setexposuremode(camera, V4L2_EXPOSURE_AUTO);
    setgainmode(camera, 1);
}
...
int dqbuf(camera_hal *camera)
{
	...
	camera->buffers[buf.index].phy_addr = (void *)buf.m.planes[0].m.mem_offset;
	camera->buffers[buf.index].bytes_used = buf.bytesused;
